    ...
    public void loginOrRegister(String username) {
        ...
        Gdx.net.sendHttpRequest(httpRequest, new Net.HttpResponseListener() {
            ...
            public void handleHttpResponse(Net.HttpResponse httpResponse) {
                if (httpResponse.getStatus().getStatusCode() == 200 || httpResponse.getStatus().getStatusCode() == 201) {
                    String response = httpResponse.getResultAsString();

                    PlayerResponse player = json.fromJson(PlayerResponse.class, response);

                    // Simpan data login ke GameManager
                    GameManager.getInstance().setPlayerId(player.playerId);
                    GameManager.getInstance().setUsername(player.username);

                    System.out.println("Login Success! ID: " + player.playerId);
                } else {
                    System.out.println("Login Error Code: " + httpResponse.getStatus().getStatusCode());
                }
            }
